...
while (ishandle(f))
    ...
    test_ra_e = ForwardKin(p.l1,p.l2,z1(1),z1(3));
    disp("dx " + (p.xtarget) + " dy " + (p.ytarget) + " x " + test_ra_e(1) + " " + test_ra_e(2));
...
function MousePos(varargin)
    ...
    if figData.tarControl
        ...
        figData.Fx = 1*(mousePos(1,1)-figData.xend);
        figData.Fy = 1*(mousePos(1,2)-figData.yend);
        disp("Fx " + Fx + " Fy + " + Fy);
    end
     set(varargin{1},'UserData',figData);
end