...
Px = simplify(T02(2, 4));
Py = simplify(T02(1, 4)); % this is forward kinematics we should use it later

P = [Px; Py; 0];

P = [- L2*sin(th1 + th2) - L1*sin(th1);
  L2*cos(th1 + th2) + L1*cos(th1);
                                0];
                                
matlabFunction(P, 'file', 'ForwardKin'); % by converting the function ra_e into a matlabFunction file, we can call it and give it parameters that we want and it would return the result instead of having to modify the variables in this code that ra_e depends on
...
B = [ -m2*L1*L2*(2*thdot1*thdot2+thdot2^2)*sin(th2)   ;
       -m2*L1*L2*thdot1*thdot2*sin(th2)                    ];
...
Zero = M * [thdotdot1;thdotdot2] + B + G + [T1;T2] - F.*[Px; Py]; %- (F - J'*[T1, T2, 0]'); % we pulled everything to one side because the solve() function will set the other side to zero to obtain thetadotdot1 and thetadotdot2
...
Imp = Kp*(desiredP - P) + Kd*(desiredPdot - J*[thdot1 thdot2]'); % J*[thdot1 thdot2]' must be recalculated for every iteration of the system because they change
